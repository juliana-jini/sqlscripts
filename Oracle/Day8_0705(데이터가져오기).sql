...
--		주의사항 : 자동으로 생성되는 테이블 컬럼의 크기가 데이터보다 작지 않게 또는 적당한 타입으로 설정해야 함.

-- ANIMAL_INS 테이블 행이 80187개 정도되면 sql 쿼리의 실행속도 차이를 확인할 수 있습니다.
...
-- 프로그래머스 문제에서 필요한 오라클 함수 : mvl (null value) 널값을 대체하는 문자열 지정
--							decode 컬럼값에 따라 출력 문자열을 지정
--							안배운 함수는 찾아보면서 하세요. 문자열 관련 함수 등등...
